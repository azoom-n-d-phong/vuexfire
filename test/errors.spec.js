...
test("bind non existing key", t => {
  ...
  t.is(
    err.message,
    `VuexFire: cannot bind undefined property 'foo'. Define it on the state first.`
  );
});
...
test("unbind non existing key", t => {
  ...
  t.is(
    err.message,
    `VuexFire: cannot unbind 'options' because it wasn't bound.`
  );
});
